...
`marketstat` method on EVE Central.

Examples:

=over 4

=item * L<http://api.eve-central.com/api/marketstat?typeid=34&typeid=35&regionlimit=10000002>

=item * L<http://api.eve-central.com/api/marketstat?typeid=34&usesystem=30000142>
...
sub _build__path {
  ...
  my $type_ids = $self->type_id;
  if (ref $type_ids eq 'ARRAY') {
